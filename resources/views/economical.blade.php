...
<section class="center-area" data-aos="zoom-in-up" data-aos-duration="1000">
	<section class="home-inner" id="economic-development">
		...
		<div class="eco-dev-right">
			...
			<span class="icons mr1 download-btn eco-dev-excel" title="Download Excel"><i
				...
				<table cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							...
							<th colspan="3" class="t-center border-left border-bottom">Avg. Weekly Wages -<br>Private
							</th>
							<th colspan="3" class="t-center border-left border-bottom">Private Businesses</th>
							<th colspan="3" class="t-center border-left border-bottom">Initial Unemployment<br>Insurance
								Claims</th>
						</tr>
						<tr>
							<th>&nbsp;</th>
					...
					<tbody id="labor-table-tbody">
						...
						<tr>
							...
							<td>{{$labor->total_nonfarm->LastYear ?? 'n.a'}}<i
									class="fa {{$labor->total_nonfarm->sign ?? 'n.a'}}"></i></td>
							<td>{{$labor->weekly->Value ?? 'n.a'}}</td>
							<td>{{$labor->weekly->Date ?? 'n.a'}}</td>
							<td>{{$labor->weekly->LastYear ?? 'n.a'}}<i
									class="fa {{$labor->weekly->sign ?? 'n.a'}}"></i></td>
							<td>{{$labor->pri_business->Value ?? 'n.a'}}</td>
							<td>{{$labor->pri_business->Date ?? 'n.a'}}</td>
							<td>{{$labor->pri_business->LastYear ?? 'n.a'}}<i
									class="fa {{$labor->pri_business->sign ?? 'n.a'}}"></i></td>
							<td>{{$labor->unemployment->Value ?? 'n.a'}}</td>
							<td>{{$labor->unemployment->Date ?? 'n.a'}}</td>
							<td>{{$labor->unemployment->LastYear ?? 'n.a'}}<i
									class="fa {{$labor->unemployment->sign ?? 'n.a'}}"></i></td>
						</tr>
						@endforeach
					</tbody>
				...
				<table cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							...
							<th colspan="3" class="t-center border-left border-bottom">Office Market Avg.<br>Asking
								Rents PSF</th>
							<th colspan="3" class="t-center border-left border-bottom">Industrial Market Avg.<br>Asking
								Rents PSF</th>
							<th colspan="3" class="t-center border-left border-bottom">Existing Single-Family
								Home<br>Median Sales Price</th>
						</tr>
						<tr>
							<th>&nbsp;</th>
							<th class="t-left">Location</th>
							<th>Current</th>
							<th>Date</th>
							<th>Change</th>
							<th>Current</th>
					...
					<tbody id="real-estate-table-tbody">
						...
						<tr>
							...
							<td>{{$real_state->office->LastYear ?? 'n.a'}} <i
									class="fa {{$real_state->office->sign ?? 'n.a'}}"></i></td>
							<td>{{$real_state->industrial->Value ?? 'n.a'}}</td>
							<td>{{$real_state->industrial->Date ?? 'n.a'}}</td>
							<td>{{$real_state->industrial->LastYear ?? 'n.a'}} <i
									class="fa {{$real_state->industrial->sign ?? 'n.a'}}"></i></td>
							<td>{{$real_state->existing->Value ?? 'n.a'}}</td>
							<td>{{$real_state->existing->Date ?? 'n.a'}}</td>
							<td>{{$real_state->existing->LastYear ?? 'n.a'}} <i
									class="fa {{$real_state->existing->sign ?? 'n.a'}}"></i></td>
						<tr>
							<td></td>
				...
				<table cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							...
							<th colspan="3" class="t-center border-left border-bottom">Commercial Electric Rates<br>(per
								Kwh)</th>
							<th colspan="3" class="t-center border-left border-bottom">Industrial Natural Gas<br>Rates
								(per 1000 Cu.Ft.)</th>
							<th colspan="3" class="t-center border-left border-bottom">Commercial Natural Gas<br>Rates
								(per 1000 Cu.Ft.)</th>
						</tr>
						<tr>
							<th>&nbsp;</th>
					...
					<tbody id="utilities-table-tbody">
						...
						<tr>
							...
							<td>{{$utilitie->ind_elec->LastYear ??''}}<i
									class="fa {{$utilitie->ind_elec->sign ??''}}"></i></td>
							<td>{{$utilitie->com_elec->Value ??''}}</td>
							<td>{{$utilitie->com_elec->Date ??''}}</td>
							<td>{{$utilitie->com_elec->LastYear ??''}}<i
									class="fa {{$utilitie->com_elec->sign ??''}}"></i></td>
							<td>{{$utilitie->ind_nat->Value ??''}}</td>
							<td>{{$utilitie->ind_nat->Date ??''}}</td>
							<td>{{$utilitie->ind_nat->LastYear ??''}}<i
									class="fa {{$utilitie->ind_nat->sign ??''}}"></i></td>
							<td>{{$utilitie->com_nat->Value ??''}}</td>
							<td>{{$utilitie->com_nat->Date ??''}}</td>
							<td>{{$utilitie->com_nat->LastYear ??''}}<i
									class="fa {{$utilitie->com_nat->sign ??''}}"></i></td>
						</tr>
						@endforeach
					</tbody>
				</table>
			...
			<div class="taxes-table tables" style="display:none;">
				<table cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							<th class="no-color">&nbsp;</th>
							<th colspan="3" class="t-center border-left border-bottom">Sales Tax rate<br>(state minimum)
							</th>
							<th colspan="3" class="t-center border-left border-bottom">Individual Income Tax
								Rate<br>(Highest bracket)</th>
							<th colspan="3" class="t-center border-left border-bottom">Corporate Income Tax
								Rate<br>(Highest bracket)</th>
						</tr>
						<tr>
							<th>&nbsp;</th>
					...
					<tbody id="taxes-table-tbody">
						...
						<tr>
							...
							<td>{{$tax->individual->LastYear ?? ''}} <i class="fa {{$tax->individual->sign ?? ''}}"></i>
							</td>
							<td>{{$tax->corporate->Value ?? ''}}</td>
							<td>{{$tax->corporate->Date ?? ''}}</td>
							<td>{{$tax->corporate->LastYear ?? ''}} <i class="fa {{$tax->corporate->sign ?? ''}}"></i>
							</td>
						</tr>
						@endforeach
					</tbody>
